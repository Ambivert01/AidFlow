...
export default function NgoCampaignDetails() {
  ...
  if (!campaign) return <div className="alert alert-danger">Mission not found or unauthorized.</div>;
    ...
    <div className="stack-lg">
        ...
        <div className="row-between items-start">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className="badge badge-primary">{campaign.disasterType}</span>
              <span className={`badge ${campaign.status === 'ACTIVE' ? 'badge-green' : 'badge-yellow'}`}>
                {campaign.status}
              </span>
            </div>
            <h1 className="page-title">{campaign.title}</h1>
            <p className="page-subtitle">{campaign.location?.district}, {campaign.location?.state}</p>
          </div>
          <div className="flex gap-3">
             <Link to="/ngo/dashboard" className="btn btn-ghost btn-sm">Mission Overview</Link>
      ...
      <div className="grid-3 mb-8">
        <div className="stat-card">
          ...
          <div className="stat-card-value">₹{(campaign.totalDonated || 0).toLocaleString("en-IN")}</div>
        ...
        <div className="stat-card">
          ...
          <div className="stat-card-value text-green-600">₹{(campaign.totalSpent || 0).toLocaleString("en-IN")}</div>
           ...
           <div className="card bg-slate-50 border-slate-100 p-5">
              ...
              <div className="stack-sm">
                <div className="row-between text-xs py-2 border-b border-slate-200 border-opacity-50">
                   ...
                   <span className="font-bold">₹{campaign.policySnapshot?.maxPerTransaction}</span>
                </div>
                <div className="row-between text-xs py-2 border-b border-slate-200 border-opacity-50">
                ...
                <div className="row-between text-xs py-2 border-b border-slate-200 border-opacity-50">
                   ...
                   <div className="flex gap-1 flex-wrap justify-end">
                      {campaign.policySnapshot?.allowedCategories.map(cat => (
                        <span key={cat} className="px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded-sm font-bold text-[9px]">{cat}</span>
                      ))}
                   </div>
                </div>
                <div className="row-between text-xs py-2">
                   <span className="text-slate-500">Min. AI Threshold</span>
                   <span className="font-bold">{(campaign.policySnapshot?.minEligibilityConfidence * 100).toFixed(0)}%</span>
                </div>
              </div>
           </div>

           <div className="card p-5 bg-primary bg-opacity-5 border-primary border-opacity-10">
              <h4 className="text-xs font-bold text-primary-dark mb-2">Workflow Trace ID</h4>
              <div className="text-[10px] font-mono text-primary break-all opacity-80">{campaign.jobIdHash}</div>
           </div>
        </div>
        ...
        <div className="col-span-8">
           ...
           {activeTab === 'beneficiaries' ? (
              ...
              <div className="card p-8 text-center text-slate-400">
                ...
                <p>Activity logs for this mission are being finalized on the blockchain.</p>
              </div>
           )}
        </div>
      </div>
