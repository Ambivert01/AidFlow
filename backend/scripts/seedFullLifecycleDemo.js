...
async function pollUntil(label, checkFn, { timeoutMs = 20000, intervalMs = 1000 } = {}) {
...
async function main() {
  ...
  await step("Remove previous demo data", async () => {
    ...
    const demoCampaigns = await Campaign.find({ createdBy: { $in: demoUserIds } });
  ...
  for (const n of ["Asha Mehta", "Ravi Kumar", "Priya Sharma"]) {
    ...
    const user = await step(`Register donor: ${n}`, () =>
      authService.registerUser({ name: n, email, password: PASSWORD, role: "DONOR" }),
    );
  ...
  await step("Admin approves NGO", () => adminService.approveUser(ngo._id, admin._id));

  const merchantUser = await step("Register merchant: Sunrise General Store", () =>
    authService.registerUser({
  ...
  await step("Admin approves government account", () => adminService.approveUser(gov._id, admin._id));

  // =========================================================
  section("PHASE 2 — NGO creates a campaign, admin approves it");
  // =========================================================
  const campaignCreate = await step("NGO creates campaign: Pune Flood Relief 2026", () =>
  ...
  const benA = await step("NGO registers beneficiary: Lakshmi Devi (no account)", () =>
    beneficiaryService.registerBeneficiary(ngo._id, {
      ...
      location: { state: "Maharashtra", district: "Pune", ward: "Ward 12" },
      household: { familySize: 5, dependents: 3, elderlyCount: 1, childrenCount: 2, disabledMembers: 0 },
  ...
  const benBUser = await step("Beneficiary self-registers account: Suresh Patil", () =>
    authService.registerUser({
      name: "Suresh Patil",
  ...
  const benB = await step("Beneficiary self-applies to campaign", () =>
    beneficiaryService.applyAsBeneficiary(beneficiaryBUserDoc._id, {
      ...
      name: "Suresh Patil",
      phone: "9123456780",
      location: { state: "Maharashtra", district: "Pune", ward: "Ward 8" },
      household: { familySize: 3, dependents: 1, elderlyCount: 0, childrenCount: 1, disabledMembers: 0 },
  ...
  await optionalStep("Wait for AI eligibility evaluation (needs workers+AI agents running)", () =>
    pollUntil("beneficiary status past PENDING", async () => {
      const a = await Beneficiary.findById(beneficiaryA._id);
      const b = await Beneficiary.findById(beneficiaryB._id);
      return a.status !== "PENDING" && b.status !== "PENDING" ? { a, b } : null;
    }),
  );
  ...
  await optionalStep("NGO approves beneficiary Lakshmi Devi", async () => {
    ...
    if (fresh.status !== "UNDER_REVIEW") {
      throw new Error(`status is ${fresh.status}, not UNDER_REVIEW - AI evaluation likely hasn't run`);
    }
    return beneficiaryService.approveBeneficiaryByNGO(beneficiaryA._id, ngo._id);
  });
  await optionalStep("NGO approves beneficiary Suresh Patil", async () => {
    ...
    return beneficiaryService.approveBeneficiaryByNGO(beneficiaryB._id, ngo._id);
  });

  // =========================================================
  section("PHASE 4 — Donations: normal, repeat-to-same-beneficiary, rejected, and escalated");
  // =========================================================

  const donation1 = await step("Donor Asha donates ₹5,000", () =>
    donationService.createDonation(donors[0]._id, { campaignId: campaign._id.toString(), amount: 5000 }),
  );
  const donation2 = await step("Donor Ravi donates ₹8,000", () =>
    donationService.createDonation(donors[1]._id, { campaignId: campaign._id.toString(), amount: 8000 }),
  );
  const donationReject = await step("Donor Priya donates ₹2,000 (will be rejected)", () =>
    donationService.createDonation(donors[2]._id, { campaignId: campaign._id.toString(), amount: 2000 }),
  );
  ...
  await optionalStep("Fire rapid donations from Asha to trigger AI escalation", async () => {
    for (let i = 0; i < 5; i++) {
  ...
  await optionalStep("Wait for AI risk evaluation on all donations", () =>
    pollUntil("donations past AI_CHECK_PENDING", async () => {
  ...
  await optionalStep("NGO assigns & approves Asha's ₹5,000 donation to Lakshmi Devi", async () => {
    await ngoService.assignDonationToBeneficiary(donation1._id, beneficiaryA._id, ngo._id);
  ...
  await optionalStep(
    ...
    async () => {
      await ngoService.assignDonationToBeneficiary(donation2._id, beneficiaryA._id, ngo._id);
      return ngoService.approveDonation(donation2._id, ngo._id);
    },
  );

  await optionalStep("NGO rejects Priya's ₹2,000 donation", () =>
    ngoService.rejectDonation(donationReject._id, ngo._id, "Campaign has sufficient funding for this phase"),
  );

  // Escalation path - tests the Phase 2 government-review fix
  await optionalStep("Government reviews the escalated donation (tests the escalation-path fix)", async () => {
    if (!escalatedDonation) throw new Error("no escalated donation was created above");
    const fresh = await Donation.findById(escalatedDonation._id);
    if (fresh.status !== "HIGH_RISK_ESCALATED") {
      throw new Error(`status is ${fresh.status}, not HIGH_RISK_ESCALATED - didn't actually trigger escalation this run`);
    }
    ...
    await ngoService.assignDonationToBeneficiary(escalatedDonation._id, beneficiaryB._id, ngo._id);
  ...
  const walletA = await optionalStep("Fetch Lakshmi Devi's wallet", async () => {
    const w = await Wallet.findOne({ beneficiary: beneficiaryA._id, campaign: campaign._id });
    if (!w) throw new Error("no wallet exists yet - donation approval step above likely didn't complete");
    return w;
  });
  if (walletA) {
    console.log(
  ...
  const walletB = await optionalStep("Fetch Suresh Patil's wallet", async () => {
    const w = await Wallet.findOne({ beneficiary: beneficiaryB._id, campaign: campaign._id });
    if (!w) throw new Error("no wallet exists yet");
    return w;
  });

  if (walletB) {
    await optionalStep("Beneficiary generates QR & merchant confirms ₹800 FOOD payment (Suresh Patil)", async () => {
      const { qrToken } = (
        await paymentService.generateQRToken(beneficiaryBUserDoc._id, walletB._id.toString())
      ).data;
  ...
  await optionalStep("NGO uploads proof of distribution", async () => {
    ...
    return proofService.createProof(
      ...
      [{ buffer: fakeImageBuffer, mimetype: "image/png", originalname: "distribution.png", size: fakeImageBuffer.length }],
    );
  });
  ...
  // if anything exists.
  await optionalStep("Check for any fraud alerts/cases generated by the above activity", async () => {
  ...
  // =========================================================
  await optionalStep("Wait for blockchain anchoring on the first donation", () =>
    pollUntil("donation1.blockchainAnchored", async () => {
      const d = await Donation.findById(donation1._id);
      return d.blockchainAnchored ? d : null;
    }, { timeoutMs: 15000 }),
  );

  await step("Query public platform stats", () => publicService.getPublicStats());
  await step("Query public blockchain status", () => publicService.getBlockchainStatus());
  ...
  if (results.skipped.length) {
    console.log("Skipped steps (likely means workers/AI agents/hardhat weren't running):");
  ...
  console.log("Login credentials for manual UI testing (all passwords: " + PASSWORD + "):");
  ...
  console.log(`  Donors:     asha${DEMO_DOMAIN}, ravi${DEMO_DOMAIN}, priya${DEMO_DOMAIN}`);
  console.log(`  Beneficiary (self-registered, has login): suresh${DEMO_DOMAIN}`);
  console.log(`  (Lakshmi Devi was NGO-registered and has no login - by design, see docs)`);
